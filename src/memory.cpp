...
void memory::write_to_memory(size_t address, int data)
{
	if(!points_to_constant(address))
		(*_block)[address] = data;
}
...
bool memory::is_address_valid(size_t address, const char* section)
{
...
void memory::set_constantAddress(size_t address, int constant)
{
	write_to_memory(address, constant); //AW: sets in memory
